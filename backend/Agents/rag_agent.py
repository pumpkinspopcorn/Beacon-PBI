...
def rag_search_function(query: Annotated[str, "The search query to find relevant documents"]) -> str:
    ...
    try:
        ...
        for i, doc in enumerate(docs, 1):
            ...
            # Extract blob URL (metadata_storage_path is the field name in the index)
            blob_url = metadata.get('metadata_storage_path') or metadata.get('metadata_url') or metadata.get('storage_path') or metadata.get('url') or 'URL not available'
