...
export const PDFViewerWithHighlight: React.FC<PDFViewerWithHighlightProps> = ({
  ...
  /**
   * Normalize chunk page numbers against a loaded PDF.
   * If the backend provides 0-based pages (or invalid pages), treat them as unknown (null)
   * so we can search across pages by text instead of silently never highlighting.
   */
  const normalizeChunkPagesForPdf = useCallback((chunksToNormalize: ChunkData[], pdfNumPages: number): ChunkData[] => {
    return chunksToNormalize.map((c) => {
      if (c.page === null) return c;
      // If page is 0-based or invalid, force null so searchForNullPageChunks can resolve it.
      if (c.page <= 0 || c.page > pdfNumPages) {
        return { ...c, page: null };
      }
      return c;
    });
  ...
  useEffect(() => {
    const loadPDF = async () => {
      try {
        ...
        // Normalize chunk pages now that we know the PDF page count
        const normalizedRawChunks = normalizeChunkPagesForPdf(rawChunks, pdf.numPages);
        ...
        const initialPage = getInitialPage(normalizedRawChunks, pdf.numPages);
        ...
        await searchForNullPageChunks(pdf, normalizedRawChunks);
      } catch (err: any) {
    ...
    loadPDF();
  }, [url, rawChunks, normalizeChunkPagesForPdf]);
