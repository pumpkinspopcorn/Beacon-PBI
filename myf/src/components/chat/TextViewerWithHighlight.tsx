...
export const TextViewerWithHighlight: React.FC<TextViewerWithHighlightProps> = ({
  ...
  const highlightedContent = useMemo(() => {
    ...
    finalChunks.forEach((chunk, index) => {
      ...
      const searchText = chunk.text.toLowerCase();
      const contentLower = textContent.toLowerCase();
      let pos = 0;

      while (pos < contentLower.length) {
        const foundIndex = contentLower.indexOf(searchText, pos);
        if (foundIndex === -1) break;

        highlights.push({
          start: foundIndex,
          end: foundIndex + chunk.text.length,
          chunkIndex: index,
        });

        pos = foundIndex + chunk.text.length;
      }
    });
